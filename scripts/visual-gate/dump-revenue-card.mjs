...
const html = await page.evaluate(() => {
  const cards = [...document.querySelectorAll('main .rounded-xl.border')]
  const revenue = cards.find((c) => c.textContent?.includes('Revenue') || c.textContent?.includes('收入'))
  return revenue?.outerHTML?.slice(0, 4500)
})
console.log(html)
await browser.close()
