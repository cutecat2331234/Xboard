...
async function probe(base) {
  ...
  const info = await p.evaluate(() => {
    ...
    const pwd = cards[1]
    const notify = cards[2]
    const reset = cards[3]
    return {
      pwdHtml: pwd?.querySelector('.n-card-content,.n-card__content')?.innerHTML?.slice(0, 800),
      notifyHtml: notify?.querySelector('.n-card-content,.n-card__content')?.innerHTML?.slice(0, 800),
      resetHtml: reset?.querySelector('.n-card-content,.n-card__content')?.innerHTML?.slice(0, 500),
    }
  })
