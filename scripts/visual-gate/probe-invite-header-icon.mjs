...
async function probe(base) {
  ...
  const info = await p.evaluate(() => {
    const extra = document.querySelector('.n-card-header__extra, .n-card-header-extra')
    const svg = extra?.querySelector('svg')
    return {
      extraHtml: extra?.innerHTML?.slice(0, 300),
      svgBox: svg?.getBoundingClientRect(),
      svgCls: svg?.getAttribute('class'),
      svgFs: svg ? getComputedStyle(svg).fontSize : null,
