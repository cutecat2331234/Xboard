...
| 端口 | 实例 | 前端 | URL |
|------|------|------|-----|
| **7001** | `/opt/xboard-legacy` | 原版闭源 dist 快照 | http://127.0.0.1:7001 |
| **7002** | `/opt/xboard` | **同一套原版 dist**（非 Vue 仿写） | http://127.0.0.1:7002 |
...
安装时生成的 `secure_path` 对两端口相同，例如：`http://127.0.0.1:7001/{secure_path}` 与 `7002/{secure_path}`
