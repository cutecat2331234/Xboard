export default {
  login: '登入',
  register: '注册',
  email: '邮箱',
  password: '密码',
  forgotPassword: '忘记密码',
  resetPassword: '重置密码',
  confirmPassword: '再次输入密码',
  emailCode: '邮箱验证码',
  sendCode: '发送',
  inviteCode: '邀请码（选填）',
  inviteCodeRequiredPh: '邀请码（必填）',
  inviteCodeRequired: '请填写邀请码',
  backToLogin: '返回登入',
  mailLinkLogin: '邮件链接登录',
  mailLinkSend: '发送登录链接',
  mailLinkSent: '登录链接已发送，请查收邮件',
  backToPasswordLogin: '密码登录',
  passwordMismatch: '两次输入的密码不一致',
  termsPrefix: '我已阅读并同意',
  termsLink: '服务条款',
  termsRequired: '请先同意服务条款',
  auth: {
    defaultTitle: 'Xboard',
    defaultDescription: '专业的网络加速服务',
    or: '或',
    loginFailed: '登录失败',
    registerFailed: '注册失败',
    validation: {
      email_required: '邮箱不能为空',
      email_format: '邮箱格式不正确',
      password_required: '密码不能为空',
    },
  },
  nav: {
    dashboard: '仪表盘',
    knowledge: '使用文档',
    order: '我的订单',
    invite: '我的邀请',
    giftCard: '礼品卡',
    plan: '购买订阅',
    node: '节点状态',
    traffic: '流量明细',
    ticket: '我的工单',
    profile: '个人中心',
    groupBilling: '财务',
    groupSubscription: '订阅',
    groupAccount: '用户',
  },
  dashboard: {
    mySubscription: '我的订阅',
    purchaseSubscription: '购买订阅',
    shortcut: '捷径',
    viewTutorial: '查看教程',
    viewTutorialDesc: '学习如何使用 Xboard',
    quickSub: '一键订阅',
    quickSubDesc: '快速将节点导入对应客户端进行使用',
    purchase: '购买订阅',
    purchaseDesc: '对您当前的订阅进行购买',
    problem: '遇到问题',
    problemDesc: '遇到问题可以通过工单与我们沟通',
    copyLink: '复制订阅链接',
    announcements: '公告',
    unpaidOrders: '您还有未支付的订单',
    payNow: '立即支付',
    openTickets: '{count} 个工单处理中',
    goView: '立即查看',
    trafficWarning: '流量已使用 {percent}%，建议续费',
    learnAndBuy: '了解并购买',
    expireAt: '到期时间',
    resetDay: '流量将在 {day} 天后重置',
    deviceLimit: '设备数限制：{count}',
    speedLimit: '速度限制：{speed} Mbps',
    invitedUsers: '您已邀请 {count} 位用户',
    clientImportTitle: '一键订阅',
    clientImportHint: '选择客户端导入订阅，可按协议类型筛选',
    noSubscribeUrl: '暂无可用订阅链接，请先购买套餐',
    popupNoticeTags: '弹窗',
    protocolTypes: {
      auto: '自动',
      anytls: 'Anytls',
      vless: 'Vless',
      hysteria: 'Hy1',
      hysteria2: 'Hy2',
      shadowsocks: 'Shadowsocks',
      vmess: 'Vmess',
      trojan: 'Trojan',
    },
  },
  plan: {
    chooseTitle: '选择最适合你的计划',
    name: '套餐',
    viewDetail: '查看详情',
    selectPeriod: '选择周期',
    coupon: '优惠券',
    couponPh: '请输入优惠券码',
    applyCoupon: '验证',
    couponApplied: '优惠券可用',
    couponDiscount: '优惠',
    buyNow: '立即购买',
    noPeriod: '当前套餐无可购周期',
    pendingOrderTitle: '未完成订单',
    pendingOrderDesc: '您有一笔待支付订单，是否取消后继续？',
    processingOrderDesc: '您有一笔订单正在开通中，请稍后再试或前往订单页查看。',
    cancelPending: '取消订单',
    periodMonth: '月付',
    periodQuarter: '季付',
    periodHalfYear: '半年付',
    periodYear: '年付',
    periodTwoYear: '两年付',
    periodThreeYear: '三年付',
    periodOnetime: '一次性',
    periodReset: '流量重置包',
    empty: '暂无可用套餐',
    loadFailed: '加载套餐失败',
    periodPricesHint: '周期价格',
    capacityRemaining: '剩余 {count} 个名额',
    tryOutBadge: '试用套餐',
    tryOutHint: '此套餐为注册试用套餐，新用户注册时将自动获得试用时长；您也可以在此正常下单购买。',
  },
  node: {
    alert: '暂无可用节点，若您未订阅或订阅已过期，请',
    empty: '暂无可用节点，若您未订阅或订阅已过期，请订阅',
    emptyPrefix: '没有可用节点，如果您未订阅或已过期请',
    emptySuffix: '。',
    subscribe: '订阅',
    name: '名称',
    type: '类型',
    rate: '倍率',
    status: '状态',
    statusHint: '五分钟内节点在线情况',
    rateHint: '使用的流量将乘以倍率进行扣除',
    tags: '标签',
    online: '在线',
    offline: '离线',
  },
  knowledge: {
    searchPh: '搜索文档…',
    empty: '暂无相关内容',
    allCategory: '全部',
    catGeneral: '通用',
    catTutorial: '教程',
    catFaq: '常见问题',
    catNotice: '公告',
  },
  errors: {
    giftCardRedeemFailed: "兑换失败，请稍后重试",
    giftCardCodeEmpty: '请输入兑换码',
    giftCardCodeMin: '兑换码长度不能少于8位',
    giftCardCodeMax: '兑换码长度不能超过32位',
    tradeNoEmpty: '订单号不能为空',
    sessionIdEmpty: '会话ID不能为空',
    couponPerUserLimit: "每人只能使用此优惠券 {limit} 次",
    couponNotStarted: "优惠券尚未开始",
    incorrectCredentials: '邮箱或密码错误',
    accountSuspended: '该账户已被停止使用',
    passwordTooManyAttempts: '密码错误次数过多，请 {minute} 分钟后再试',
    tokenError: '登录链接无效或已过期',
    userNotFound: '用户不存在',
    invalidRequest: '无效请求',
    sendingTooFrequent: '发送频繁，请稍后再试',
    emailSuffixNotAllowed: '邮箱后缀不在白名单中',
    gmailAliasNotSupported: '不支持 Gmail 别名邮箱',
    registrationClosed: '本站已关闭注册',
    inviteCodeRequired: '必须使用邀请码才可以注册',
    emailCodeRequired: '邮箱验证码不能为空',
    incorrectEmailCode: '邮箱验证码有误',
    emailExists: '邮箱已在系统中存在',
    invalidInviteCode: '邀请码无效',
    registerFailed: '注册失败',
    registerTooFrequent: '注册频繁，请等待 {minute} 分钟后再次尝试',
    invalidCaptcha: '验证码有误',
    invalidCaptchaType: '验证码配置无效',
    invalidEmailVerificationPurpose: '无效的邮箱验证用途',
    emailCodeSentRecently: '验证码已发送，请过一会儿再请求',
    emailNotRegistered: '该邮箱不存在系统中',
    resetFailed: '重置失败',
    resetTooFrequent: '重置失败，请稍后再试',
    unauthorized: '授权失败，请先登录',
    sessionExpired: '账号信息已过期，请重新登录',
    sessionKicked: '账号在其他设备登录，请重新登录',
    loginFailed: '登录失败',
    orderNotFound: '订单不存在',
    orderNotFoundOrPaid: '订单不存在或已支付',
    planNotFound: '订阅计划不存在',
    paymentMethodUnavailable: '支付方式不可用',
    paymentNotFound: '未找到可用的 Stripe 支付方式',
    requestFailed: '请求失败，请稍后再试',
    commConfigFailed: '站点配置加载失败，部分功能可能不可用，请刷新页面重试',
    tooManyAttempts: '尝试次数过多，请稍后再试',
    telegramLoginUnavailable: 'Telegram 登录不可用',
    featureDisabled: '该功能已关闭',
    sessionNotFound: '会话不存在或已失效。',
    recordNotFound: '记录不存在。',
    paymentFailed: '支付失败',
    invalidParameter: '参数错误',
    cancelPendingOnly: '只可以取消待支付订单',
    cancelFailed: '取消失败',
    unpaidOrderExists: '您有未付款或开通中的订单，请稍后再试或将其取消',
    insufficientBalance: '余额不足',
    paymentCardFailed: '扣款失败，请检查信用卡信息',
    paymentGatewayFailed: '支付网关请求失败',
    paymentInProgress: '订单支付进行中，无法取消',
    paymentMethodLocked: '请取消订单并重新创建以更换支付方式',
    currencyConversionTimeout: '货币转换超时，请稍后再试',
    ticketWaitForReply: '请等待客服回复后再发送消息',
    withdrawUnsupported: '不支持提现',
    insufficientCommission: '佣金余额不足',
    pendingWithdrawTicket: '您已有待处理的提现申请',
    withdrawMinimum: '当前最低提现金额为 {limit}',
    serverError: '遇到了些问题，我们正在进行处理',
    pageNotFound: '页面不存在',
    planChangeDisabled: '当前不允许更改订阅，请联系客服或提交工单',
    withdrawMethodUnsupported: '不支持的提现方式',
    wrongPlanPeriod: '套餐周期参数有误',
    withdrawMethodEmpty: '提现方式不能为空',
    withdrawAccountEmpty: '提现账号不能为空',
    invalidCoupon: '优惠券无效',
    couponEmpty: '请输入优惠券码',
    couponExpired: '优惠券已过期',
    couponPeriodMismatch: '优惠券不可用于该订阅周期',
    couponFailed: '优惠券使用失败',
    couponUsedUp: '优惠券不可用',
    couponPlanMismatch: '优惠券不可用于该订阅',
    planSoldOut: '当前商品已售罄',
    planPeriodUnavailable: '该订阅周期不可购买',
    planRenewDisabled: '该订阅不可续费',
    resetTrafficNoSub: '需要有效订阅才能购买流量重置包',
    orderCreateFailed: '创建订单失败',
    checkoutFailed: '结算失败',
    openTicketExists: '您已有未关闭的工单，请先关闭后再创建',
    ticketNotFound: '工单不存在',
    ticketMessageEmpty: '消息不能为空',
    ticketClosed: '工单已关闭',
    ticketClosedReply: '工单已关闭，无法回复',
    ticketReplyFailed: '工单回复失败',
    ticketCloseFailed: '关闭失败',
    ticketSystemDisabled: '工单系统已关闭',
    withdrawTicketCloseForbidden: '提现工单不能自行关闭，请联系客服',
    ticketSubjectEmpty: '工单主题不能为空',
    oldPasswordWrong: '旧密码有误',
    passwordTooShort: '密码至少需要 8 位',
    saveFailed: '保存失败',
    transferFailed: '划转失败',
    telegramNotBound: '未绑定 Telegram 账号',
    telegramUnbindFailed: '解绑 Telegram 失败',
    inviteCodeLimit: '已达到邀请码创建上限',
    giftCardNotFound: '兑换码不存在',
    giftCardDisabled: '该礼品卡类型已停用',
    giftCardIneligible: '您不满足此礼品卡的使用条件',
    giftCardLimitReached: '您已达到此礼品卡的使用限制',
    giftCardQueryFailed: '查询失败，请稍后重试',
    giftCardUnavailable: '礼品卡功能未开启',
    messageTooLong: '消息过长',
    searchKeywordTooLong: '搜索关键词过长',
    planPeriodEmpty: '套餐周期不能为空',
    recordIdRequired: '记录ID不能为空',
    commissionUpdateFailed: '更新佣金余额失败',
  },
  common: {
    loading: '加载中...',
    save: '保存',
    cancel: '取消',
    submit: '提交',
    confirm: '确认',
    logout: '退出登录',
    logoutConfirm: '确定要退出登录吗？',
    switchToLight: '切换到浅色模式',
    switchToDark: '切换到深色模式',
    selectLanguage: '选择语言',
    toggleFullscreen: '切换全屏',
    language: '简体中文',
    close: '关闭',
    actions: '操作',
    copy: '复制',
    success: '成功',
    error: '错误',
    reset: '重置',
    search: '搜索',
    pagination: {
      summary: '第 {current}/{total} 页，共 {count} 条',
      previous: '上一页',
      next: '下一页',
    },
    units: {
      b: 'B',
      kb: 'KB',
      mb: 'MB',
      gb: 'GB',
      tb: 'TB',
    },
  },
  invite: {
    title: '我的邀请',
    available: '当前剩余佣金',
    transfer: '划转',
    withdraw: '推广佣金提现',
    generate: '生成邀请码',
    codeMgmt: '邀请码管理',
    code: '邀请码',
    pv: '访问量',
    status: '状态',
    statusUnused: '未使用',
    statusUsed: '已使用',
    createdAt: '创建时间',
    copyLink: '复制链接',
    registered: '已注册用户数',
    people: '人',
    peopleCount: '{number} 人',
    commissionRate: '佣金比例',
    pendingCommission: '确认中的佣金',
    totalCommission: '累计获得佣金',
    incomeRecord: '佣金发放记录',
    incomeTime: '发放时间',
    incomeAmount: '佣金',
    transferAmount: '划转金额（元）',
    transferAmountRequired: '请输入划转金额',
    transferAmountInvalid: '划转金额须为正数，最多两位小数',
    commissionTiers: '{l1}%、{l2}%、{l3}%',
    withdrawAccountRequired: '请输入提现账号',
    withdrawMethodRequired: '请选择提现方式',
    withdrawSuccess: '提现申请已提交，请前往工单查看进度。',
    withdrawLimitHint: '最低提现金额：{limit}',
    withdrawFeeHint: '提现手续费：{rate}%',
    transferMinHint: '最低划转金额：{limit}',
    transferNetHint: '手续费 {fee}，实际到账 {net}',
    withdrawFullBalanceHint: '将提现全部可用佣金（扣除手续费后到账）',
  },
  traffic: {
    hint: '流量明细仅保留近月数据以供查询。',
    empty: '暂无流量记录',
    recordAt: '记录时间',
    upload: '实际上行',
    download: '实际下行',
    rate: '扣费倍率',
    total: '总计',
    formula: '公式：(实际上行 + 实际下行) x 扣费倍率 = 扣除流量',
  },
  order: {
    listTradeNo: '# 订单号',
    tradeNo: '订单号',
    orderType: '订单类型',
    copyTradeNo: '复制订单号',
    tradeNoCopied: '订单号已复制',
    period: '周期',
    amount: '订单金额',
    status: '订单状态',
    statusPending: '待支付',
    statusProcessing: '开通中',
    statusCanceled: '已取消',
    statusCompleted: '已完成',
    statusOffset: '已折抵',
    empty: '暂无订单',
    filterAll: '全部状态',
    createdAt: '创建时间',
    pay: '支付',
    view: '查看',
    viewDetail: '查看详情',
    detailTitle: '订单详情',
    selectPayment: '请选择支付方式',
    paymentLocked: '该订单已锁定支付方式，如需更换请先取消订单',
    redirectPaymentHint: '请在新打开的标签页中完成支付，支付成功后本页将自动更新。',
    redirectPopupBlocked: '浏览器拦截了弹窗，请允许本站弹窗后重试。',
    paymentMethod: '支付方式',
    noPaymentMethods: '暂无可用支付方式，请联系管理员或稍后再试',
    totalTitle: '订单总额',
    checkout: '结账',
    notice: '注意',
    closeOrder: '关闭订单',
    closeConfirm: '如果您已经付款，取消订单可能会导致支付失败，确定要取消订单吗？',
    closeSuccess: '取消成功',
    discount: '优惠金额',
    discountLabel: '折扣',
    surplus: '折抵',
    surplusLegacy: '旧订阅折抵金额',
    refund: '退款',
    refundAmount: '退款金额',
    balancePay: '余额支付',
    grandTotal: '总计',
    waitingPay: '等待支付中',
    productInfo: '商品信息',
    productName: '产品名称',
    typePeriod: '类型/周期',
    productTraffic: '产品流量',
    orderInfo: '订单信息',
    typeNew: '新购',
    typeRenew: '续费',
    typeUpgrade: '升级',
    typeReset: '流量重置',
    paySuccess: '支付成功',
    cancelledDuringPay: '订单已取消，支付未完成。',
    balanceDeduct: '余额抵扣',
    handlingFee: '手续费',
    payAmount: '应付金额',
    scanPay: '扫码支付',
    scanHint: '请使用对应 App 扫描二维码完成支付',
    stripeRequired: '请填写信用卡信息',
    stripeUnavailable: 'Stripe 支付暂不可用',
    stripeLoadFailed: 'Stripe 加载失败',
    stripeTokenFailed: 'Stripe 支付验证失败',
    tryOutBadge: '试用套餐',
    tryOutHint: '此订单包含注册试用套餐，支付完成后将开通对应试用权益。',
  },
  ticket: {
    title: '工单历史',
    new: '新的工单',
    subject: '主题',
    subjectPh: '请输入工单主题',
    level: '工单级别',
    levelPh: '请选项工单等级',
    status: '工单状态',
    createdAt: '创建时间',
    lastReply: '最后回复时间',
    message: '消息',
    messagePh: '请描述你遇到的问题',
    fillRequired: '请填写主题和消息',
    levelLow: '低',
    levelMedium: '中',
    levelHigh: '高',
    levelWithdraw: '提现',
    withdrawMethod: '提现方式',
    withdrawAccount: '提现账号',
    view: '查看',
    close: '关闭',
    closed: '已关闭',
    replied: '已回复',
    pendingReply: '待回复',
    open: '进行中',
    reply: '回复',
    replyPh: '输入内容回复工单',
    closedReplyPh: '工单已关闭，无法回复',
    closedHint: '工单已关闭，无法继续回复。',
    closeSuccess: '工单已关闭',
    closeConfirm: '确定关闭此工单吗？关闭后将无法继续回复。',
    empty: '暂无工单',
  },
  giftCard: {
    title: '礼品卡',
    hint: '输入兑换码可领取余额、流量或套餐等奖励，兑换记录可在下方查看。',
    redeem: '兑换礼品卡',
    historyTitle: '兑换记录',
    codePh: '请输入礼品卡兑换码',
    codeRequired: '请输入兑换码',
    recheckRequired: '兑换码已变更，请重新校验后再兑换',
    check: '查询',
    confirmRedeem: '确认兑换',
    redeemSuccess: '兑换成功',
    templateName: '礼品卡名称',
    type: '类型',
    code: '兑换码',
    codeMasked: '兑换码',
    rewards: '获得奖励',
    rewardPreview: '奖励预览',
    mysteryPreview: '随机奖励（奖池 {count} 项，兑换后揭晓）',
    rewardBalance: '余额',
    rewardTraffic: '流量',
    rewardExpireDays: '延长有效期(天)',
    rewardDeviceLimit: '设备数',
    rewardResetPackage: '重置当月流量',
    rewardPlan: '套餐',
    days: '天',
    redeemedAt: '兑换时间',
    viewDetail: '查看详情',
    detailTitle: '兑换详情',
    description: '描述',
    inviteRewards: '邀请人奖励',
    multiplier: '倍率',
    historyEmpty: '暂无兑换记录',
  },
  profile: {
    wallet: '我的钱包',
    balanceHint: '账户余额(仅用于结算)',
    email: '邮箱',
    uuid: 'UUID',
    changePassword: '修改密码',
    oldPassword: '旧密码',
    newPassword: '新密码',
    confirmNewPassword: '确认新密码',
    confirmPasswordPh: '请再次输入新密码',
    oldPasswordPh: '请输入旧密码',
    newPasswordPh: '请输入新密码',
    notify: '通知',
    remindExpire: '到期邮件提醒',
    remindTraffic: '流量邮件提醒',
    resetSubscribe: '重置订阅信息',
    resetDesc: '当你的订阅地址或账户发生泄漏被他人滥用时，可以在此重置订阅信息。避免带来不必要的损失。',
    resetConfirm: '确定重置订阅链接和令牌吗？已导入的客户端需要重新更新订阅。',
    resetSuccessWithUrl: '订阅已重置，请将以下新链接导入客户端：\n\n{url}',
    accountInfo: '账户信息',
    subscribeToken: '订阅 Token',
    activeSessions: '活跃会话',
    sessionDevice: '设备',
    sessionCreated: '登录时间',
    sessionLastUsed: '最后活跃',
    kickSession: '踢出会话',
    kickSessionConfirm: '确定要踢出设备「{device}」的登录会话吗？',
    noSessions: '暂无活跃会话',
    quickLogin: '快捷登录',
    quickLoginHint: '生成一次性登录链接，可在其他设备快速登录当前账号。',
    generateQuickLogin: '生成快捷登录链接',
    quickLoginCopied: '快捷登录链接已复制到剪贴板',
    copied: '已复制到剪贴板',
    telegram: 'Telegram',
    telegramHint: '在 Telegram 搜索机器人并发送 /bind 绑定账号：',
    telegramBound: '已绑定 Telegram',
    telegramUnbind: '解绑 Telegram',
    telegramBotUnavailable: 'Telegram 机器人暂不可用，请稍后再试。',
    telegramUnbindConfirm: '确定要解绑 Telegram 账号吗？解绑后需重新通过机器人 /bind 绑定。',
    telegramGroup: '加入 Telegram 讨论群',
    validation: {
      old_password_required: '旧密码不能为空',
      new_password_required: '新密码不能为空',
      remind_expire_format: '过期提醒参数有误',
      remind_traffic_format: '流量提醒参数有误',
    },
  },
}
