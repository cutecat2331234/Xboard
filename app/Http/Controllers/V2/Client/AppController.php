    ...
    public function getConfig(Request $request)
    {
        $config = [
            ...
            'features' => [
                ...
                'enable_ticket_system' => AppFeature::ticketEnabled(), // 是否开启工单系统
                'ticket_must_wait_reply' => (bool) admin_setting('ticket_must_wait_reply', 0), // 工单是否需要等待管理员回复后才可继续发消息
                'enable_commission_system' => AppFeature::commissionEnabled(), // 是否开启佣金系统
                'enable_traffic_log' => AppFeature::trafficLogEnabled(), // 是否开启流量日志
                'enable_knowledge_base' => AppFeature::knowledgeEnabled(), // 是否开启知识库
                'enable_announcements' => AppFeature::announcementsEnabled(), // 是否开启公告系统
                'enable_auto_renewal' => false, // 服务端自动续费尚未实现，固定返回 false
                'enable_coupon_system' => AppFeature::couponEnabled(), // 是否开启优惠券系统
